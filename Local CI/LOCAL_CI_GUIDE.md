...
- ✅ Tests pass locally
- ❌ CI fails on GitHub Actions/CircleCI
...
CI.run do
  ...
  # Final messaging
  if success?
    heading "✅ All CI checks passed!", "Ready for code review"
  else
    failure "❌ CI checks failed", "Fix issues before submitting PR"
  end
end
```
...
**Problems**:
- ❌ Configuration duplicated between local scripts and CI
- ❌ Developers may not run the same checks locally
- ❌ Easy for configurations to drift out of sync
- ❌ Adding a new check requires updating multiple places
...
**Benefits**:
- ✅ Single source of truth in `config/ci.rb`
- ✅ Guaranteed consistency between local and CI
- ✅ Adding a new check only requires updating `config/ci.rb`
- ✅ Developers test the exact same pipeline before pushing
...
CI.run do
  ...
  if success?
    ...
    failure "❌ Some checks failed", "Fix issues above"
  end
end
```
...
**Benefits**:
- ✅ Identical behavior locally and remotely
- ✅ Single source of truth for CI configuration
- ✅ Easy to debug - just run `bin/ci` locally
...
**What you'll see**:
```
❌ Intentional Failure - failed (0.1s)
...
**What you'll see**:
```
✅ Tests: Rails - passed (2.8s)  # Notice the longer time!
```
...
- ✅ Fix issues with fast feedback
- ✅ Commit only when tests pass
- ✅ Reduce CI failures on GitHub
- ✅ Save team time and infrastructure costs
...
# Example output:
# ✅ Setup - passed (2.3s)
# ✅ Tests: Rails - passed (45.2s)  # ← Too slow!
```
...
✅ **How CI works** - Sequential step execution with fast-fail behavior
✅ **What failures look like** - Error messages, stack traces, and timing
✅ **How to debug** - Isolating issues and running targeted tests
✅ **Why it's valuable** - Catching bugs before code review or production
✅ **Best practices** - Running CI before every PR, keeping tests fast
